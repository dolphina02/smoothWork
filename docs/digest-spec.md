...
그 자리에서 처리하는 화면. 상단 탭 3개로 구성된다.

- **미팅 탭**: Outlook 캘린더 일정을 목록으로 보여주고, 녹음이 있는 미팅은 STT 결과를 LLM이
  요약해서 붙인다
- **Slack 탭**: 채널별로 하루(새벽 5시 ~ 다음날 새벽 5시)를 기준으로 그 기간 오간 대화를
  LLM이 요약한다
- **Follow-up Task 탭**: 그 미팅/Slack 대화에서 나온, 아직 확정되지 않은 태스크 후보를 그
  자리에서 검토·확정한다 (기존 Inbox의 승인/수정/반려 기능을 재사용, 7절)

Inbox/Board가 "태스크로 추출된 것"에 집중한다면, 미팅/Slack 탭은 태스크로 추출되지 않은 맥락까지
포함해 "그날/그 미팅에 무슨 일이 있었는지"를 원본에 가깝게 훑어보는 역할이고, Follow-up Task
탭이 그 사이를 이어 "그래서 뭘 해야 하는가"로 연결한다. Reports가 기간 단위로 다듬어서 공유하는
...
```
┌──────────────────────────────────────────────────────────────────┐
│  ◀  2026-08-11 (어제)  ▶         [탭: 미팅 | Slack | Follow-up Task]│
├───────────────────┬──────────────────────────────────────────────┤
│ 좌측: 리스트         │ 우측: 선택 항목 상세                            │
│ (탭에 따라 내용 전환)  │ (탭에 따라 내용 전환)                          │
│                     │                                            │
│  예) 미팅 탭 선택 시:  │  예) 미팅 탭 선택 시:                          │
...
- 상단: 날짜 네비게이터(하루 단위 좌우 이동, 또는 미니 캘린더로 특정일 점프) + 탭 3개. 날짜
  네비게이터는 세 탭에 공통으로 적용된다
- **기본 진입값은 "어제"** (사이드바 라벨과 일치) — ◀▶로 그보다 이전 날짜도 계속 탐색할 수 있다
- 기본 탭은 "미팅"
- 좌측: 선택된 탭에 해당하는 리스트 (미팅 목록 / Slack 채널 목록 / Follow-up Task 후보 목록).
  기존에는 미팅과 Slack이 한 리스트 안에 섹션으로 같이 있었지만, 탭으로 분리해 셋을 명확히
  구분한다 (Inbox의 리스트-상세 패턴은 그대로 유지)
- 우측: 선택된 항목의 상세 뷰
